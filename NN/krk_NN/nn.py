...
def loadData():
    ...
    file = open('krk_NN/dataset/krkopt.data', 'r')
    i = 0
    while True:
        # for i in range(10):
        line = file.readline()
...
def testNN():
    ...
    file = open('krk_NN/model.pkl', 'wb')
    pk.dump(storedNN, file)
    file.close()
    file = open('krk_NN/xTest.data', 'wb')
    pk.dump(xTesting, file)
    file.close()
    file = open('krk_NN/yTest.data', 'wb')
...
def testFromFile():
    ...
    nn = pk.load(file)
    file.close()
    file = open('krk_NN/xTest.data', 'rb')
    xTest = pk.load(file)
    file.close()
    file = open('krk_NN/yTest.data', 'rb')
    yTest = pk.load(file)
    file.close()
    wrongs, accuracy = nn_test(nn, xTest, yTest)
    print('acc on test data: ', accuracy)
